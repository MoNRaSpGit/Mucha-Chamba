...
const UserList = () => {
    const users = [
        { id: 1, name: 'Juan', profession: 'Plomero', available: true, age: 35, experience: '+10 años' },
        { id: 2, name: 'Maria', profession: 'Plomero', available: false, age: 29, experience: '+5 años' },
        { id: 3, name: 'Pedro', profession: 'Electricista', available: true, age: 40, experience: '+15 años' },
        { id: 4, name: 'Laura', profession: 'Electricista', available: false, age: 32, experience: '+10 años' },
    ...
    const handleSelectUser = (user) => {
        ...
        // Imprimir todas las profesiones y la del seleccionado para verificar los datos
        console.log("Profesiones en el array de usuarios:", users.map((user) => user.profession));
        console.log("Profesión del seleccionado:", user.profession);

        // Paso 1: Normalizar la profesión del trabajador seleccionado
        const selectedProfession = user.profession.toLowerCase().trim();
        console.log("Profesión normalizada del seleccionado:", selectedProfession);

        // Paso 2: Filtrar los usuarios
        const filteredRecommendations = users.filter((rec) => {
            const selectedProfession = user.profession.toLowerCase().trim();
            const recProfession = rec.profession.toLowerCase().trim();

            // Depuración directa
            console.log(`Comparando seleccionado: "${selectedProfession}" con iterado: "${recProfession}"`);
    ...
    const filteredUsers = users
        .filter(user =>
            filterAvailable === null ? true : user.available === filterAvailable
        )
        .filter(user =>
